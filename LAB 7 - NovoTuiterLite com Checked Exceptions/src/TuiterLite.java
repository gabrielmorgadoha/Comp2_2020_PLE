...
public class TuiterLite<T> {
     ...
     */
    public Tuite tuitarAlgo(Usuario usuario, String texto) throws TamanhoMaximoException, UsuarioInvalidoException {

        if (!usuarioByEmail.containsKey(usuario.getEmail())){
            UsuarioInvalidoException e = new UsuarioInvalidoException();
            e.setUsuarioInvalido(usuario.getEmail());
            throw e;
        }
        if (texto.length() > TAMANHO_MAXIMO_TUITES){
            TamanhoMaximoException e = new TamanhoMaximoException();
            e.setLetrasExcedentes(texto.length() - TAMANHO_MAXIMO_TUITES);
            throw e;
        }
        else {
